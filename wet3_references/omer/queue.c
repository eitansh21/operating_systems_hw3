...
int isFull(struct Queue* q) {
    int ans = (q->currentSize == q->maxSize);
    return ans;
}
...
int pushToQueue(struct Queue* q, int data, struct timeval time) {
    if (isFull(q)) {
        //printf("shira: queue is full, q->currentSize = %d, q->maxSize %d\n",q->currentSize, q->maxSize);
        return ERROR;
    }
    struct Element* newElement = (struct Element*)malloc(sizeof(struct Element));
    if (!newElement) {
        //printf("shira: newElement = NUll");
...
void runQueueTests() {
    ...
    assert(freeQueueFunction(myQueue) == SUCCESS);
