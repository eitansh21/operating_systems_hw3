...
void requestServeDynamic(int fd, char *filename, char *cgiargs,struct Tstats* threadStats)
{
...
void requestServeStatic(int fd, char *filename, int filesize,struct Tstats* threadStats)
{
   int srcfd;
...
void requestHandle(int fd,struct timeStats* tStats,struct Tstats* threadStats)
{
   ...
   threadStats->total_req++;
    requestPrintStats(tStats,threadStats);
}
